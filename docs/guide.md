...
Если при создании очереди установить флаг `Fan-out`, будет создана очередь сообщения из которой могут получать несколько потребителей. Например, такая очередь необходима для условной трансформации сообщений.
...
Трансформация позволяет обработать входящее сообщение, получить дополнительные данные из внешнего `http` сервиса. Для создания условной обработки необходимо сделать входящий канал `fun-out`.
