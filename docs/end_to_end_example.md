...
1. **Приложение 1: `StoreApp`**
    * **Канал 1:**
        ...
        * **Режим Не удалять:** ✓ **Включен**. Это критически важно, так как мы хотим, чтобы сообщение о заказе было доступно нескольким маршрутам-фильтрам для реализации ветвления.
